...
import { ChannelType, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, PermissionsBitField } from 'discord.js';
...
//
export async function createTicketThread(interaction, ticketType, data, shouldDefer = true) {
    ...
    if (announcementChannel && announcementChannel.isTextBased()) {
        ...
        const accessButton = new ButtonBuilder()
            ...
            .setURL(thread.url);
        const announcementRow = new ActionRowBuilder().addComponents(accessButton);
...
//
export async function handleCloseThread(interaction) {
    try {
        await interaction.deferReply({ ephemeral: true });
...
export async function handleReopenThread(interaction) {
    try {
        await interaction.deferReply({ ephemeral: true });
        const channel = interaction.channel;
        if (!channel || !channel.isThread()) {
...
export async function handleCloseThreadCommand(interaction) {
    try {
        await interaction.deferReply({ ephemeral: true });
        const channel = interaction.channel;
        if (!channel || !channel.isThread()) {
        ...
        if (ticket) {
            ...
            if (ticket.added_user && Array.isArray(ticket.added_user)) {
                ...
                const addedUsers = ticket.added_user.filter((user) => typeof user === 'string');
                usersToRemove.push(...addedUsers);
            }
            // Remove all users concurrently.
            await Promise.all(usersToRemove.map(userId => thread.members.remove(userId).catch(err => {
