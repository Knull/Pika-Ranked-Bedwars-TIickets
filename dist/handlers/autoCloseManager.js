    ...
    cron.schedule('*/15 * * * * *', async () => {
        ...
        for (const ticket of tickets) {
            ...
            if (!ticket.lastMessageAt && diff >= 15 * 60 * 1000) {
                ...
                const autoCloseRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('delete_ticket_auto').setLabel('Delete').setStyle(ButtonStyle.Secondary).setEmoji('⏳'), new ButtonBuilder().setCustomId('reopen_ticket').setLabel('Reopen').setStyle(ButtonStyle.Secondary).setEmoji('🔓'));
                await channel.send({ embeds: [autoCloseEmbed], components: [autoCloseRow.toJSON()] });
