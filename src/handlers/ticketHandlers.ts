...
export async function handleClaimTicket(client: Client, interaction: ModalSubmitInteraction, reason: string): Promise<void> {
  try {
    await interaction.deferReply({ ephemeral: true });
    const ticket = await prisma.ticket.findFirst({ where: { channelId: interaction.channel?.id } });
    if (!ticket) {
    ...
    const logsChannel = logsChannelFetched as TextChannel;
    ...
    const detailsRow = new ActionRowBuilder<ButtonBuilder>().addComponents(detailsButton);
    ...
    fs.unlink(transcriptFile, (err) => {
      ...
      else console.log('Transcript file deleted.');
    });
